...
function get_data()
    ...
    elseif backend == "acpi" or backend == "acpitool" then
        ...
        while line do
            ...
            rv.time = data:match(".*, ([%d]?[%d]?:?[%d][%d])") -- no seconds
...
local function warn(state, charge)
    if state == "discharging" then
        if charge <= nextlim then
            naughty.notify({ title = "⚡ Beware! ⚡",
            screen = capi.mouse.screen,
                text = "Battery charge is low ( ⚡ ".. charge .."%)!",
                timeout = 7,
                fg = '#ffffff',
                bg =  '#ff0000'})
...
local function update()
    ...
    battery_status = lib.markup.fg.color(color, state) ..  
                        awful.util.escape(tostring(bat.charge)) .. "%"

    if bat.time then
        battery_status = battery_status .. " " .. 
                lib.markup.fg.color('#aaaa00', "~" .. awful.util.escape(bat.time))
    end

    warn(bat.state, bat.charge)
